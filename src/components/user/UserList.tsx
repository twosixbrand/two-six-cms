        ...
        >
            ...
            <div>
              <div style={{ marginBottom: '0.4rem', display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                ...
                {item.roles && item.roles.map(role => (
                  <span key={role.code_role} style={{ fontSize: '0.65rem', background: 'rgba(212,175,55,0.8)', color: '#fff', padding: '0.3rem 0.6rem', borderRadius: '6px', fontWeight: 600, letterSpacing: '0.5px' }}>
                    {role.name}
                  </span>
                ))}
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.5rem', marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--text-primary)', fontWeight: 500 }}>{item.login}</span>
