  ...
  return (
    <div className="page-container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        <SearchInput id="role-search" value={search} onChange={setSearch} placeholder="Buscar por nombre..." />
      ...
      {loading ? (
        ...
        <DataTable
          ...
          actions={(row) => (
            <>
              <Button variant="edit" size="sm" icon={<FiEdit2 />} onClick={() => openEditModal(row)} aria-label={`Editar rol ${row.name}`} />
              <Button variant="destructive" size="sm" icon={<FiTrash2 />} onClick={() => handleDelete(row)} aria-label={`Eliminar rol ${row.name}`} />
            </>
          )}
        />
      )}
